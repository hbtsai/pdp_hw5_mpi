...
int check_win(int size, int** board, int i, int j){
	...
	//up + down
	for( ; count1 <= 4 ; count1++ ){
		if( j+count1 >= size || board[i][j+count1] != who )break;
	}
	for( ; count2 <= 4 ; count2++ ){
		if( j-count2 < 0 || board[i][j-count2] != who )break;
	}
	if( count1 + count2 >= 6 ) return 1;

	//left + right
	count1 = 1;
	count2 = 1;
	for( ; count1 <= 4 ; count1++ ){
		if(i+count1 >= size || board[i+count1][j] != who) break;
	}
	for( ; count2 <= 4 ; count2++ ){
		if(i-count2 < 0 || board[i-count2][j] != who) break;
	}
	if( count1 + count2 >= 6 ) return 1;
